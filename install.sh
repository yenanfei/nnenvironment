...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    version="$(curl -fsSL https://api.kgithub.com/repos/coder/code-server/releases | awk 'match($0,/.*"html_url": "(.*\/releases\/tag\/.*)".*/)' | head -n 1 | awk -F '"' '{print $4}')"
  else
    # https://gist.github.com/lukechilds/a83e1d7127b78fef38c2914c4ececc3c#gistcomment-2758860
    version="$(curl -fsSLI -o /dev/null -w "%{url_effective}" https://kgithub.com/coder/code-server/releases/latest)"
  fi
  version="${version#https://kgithub.com/coder/code-server/releases/tag/}"
  version="${version#v}"
  echo "$version"
}
...
echo_coder_postinstall() {
  echoh
  echoh "Deploy code-server for your team with Coder: https://kgithub.com/coder/coder"
}
...
install_deb() {
  ...
  fetch "https://kgithub.com/coder/code-server/releases/download/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://kgithub.com/coder/code-server/releases/download/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://kgithub.com/coder/code-server/releases/download/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
...
#
# Inspired by https://kgithub.com/docker/docker-install/blob/26ff363bcf3b3f5a00498ac43694bf1c7d9ce16c/install.sh#L111-L120.
distro() {
